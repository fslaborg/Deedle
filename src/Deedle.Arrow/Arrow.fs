...
module private Implementation =
    ...
    let netTypeToArrowType (t: Type) : IArrowType =
    ...
    let epochDto = DateTimeOffset(1970, 1, 1, 0, 0, 0, TimeSpan.Zero)
    ...
    let columnToArrowArray (typ: Type) (vec: IVector<obj>) : IArrowArray =
    ...
    let arrowArrayToVector (arr: IArrowArray) : Type * IVector =
        let n = arr.Length
        match arr with
        | :? DoubleArray as a ->
    ...
    let batchesToFrame (batches: RecordBatch[]) : Frame<int, string> =
...
module Frame =
    ...
    let toRecordBatch (frame: Frame<'R,'C>) : RecordBatch =
        Implementation.frameToRecordBatch frame
    ...
    let ofRecordBatch (batch: RecordBatch) : Frame<int, string> =
        Implementation.recordBatchToFrame batch
    ...
    /// </summary>
    let writeArrow (path: string) (frame: Frame<'R, string>) : unit =
        Implementation.writeArrow path frame
    ...
    let readArrowStream (stream: System.IO.Stream) : Frame<int, string> =
        Implementation.readArrowStream stream
    ...
    let writeArrowStream (stream: System.IO.Stream) (frame: Frame<'R, string>) : unit =
        Implementation.writeArrowStream stream frame
    ...
    let readFeather (path: string) : Frame<int, string> =
        Implementation.readFeather path
    ...
    let writeFeather (path: string) (frame: Frame<'R, string>) : unit =
        Implementation.writeFeather path frame
    ...
    /// </summary>
    let writeArrowWithIndex (path: string) (frame: Frame<'R, string>) : unit =
        Implementation.writeArrowWithIndex path frame
    ...
    /// </summary>
    let readArrowWithIndex (path: string) : Frame<string, string> =
        Implementation.readArrowWithIndex path
    ...
    /// </summary>
    let writeFeatherWithIndex (path: string) (frame: Frame<'R, string>) : unit =
        Implementation.writeFeatherWithIndex path frame

    /// <summary>
    /// Read a Feather v2 file that was written with <c>Frame.writeFeatherWithIndex</c>,
    /// restoring the original string row keys.
    /// </summary>
    let readFeatherWithIndex (path: string) : Frame<string, string> =
        Implementation.readFeatherWithIndex path
...
module Series =
    ...
    let toArrowArray (series: Series<'K, 'V>) : IArrowArray =
        Implementation.seriesToArrowArray series
    ...
    let ofArrowArray (arr: IArrowArray) : Series<int, obj> =
        Implementation.arrowArrayToSeries arr
