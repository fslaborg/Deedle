...
let private getData (col: DataColumn) : Array = col.Data

/// Get the value count from a DataColumn.
let private getNumValues (col: DataColumn) : int = int col.NumValues
...
let private netTypeToDataField (name: string) (t: Type) : DataField =
...
let private columnToDataColumn (field: DataField) (typ: Type) (vec: IVector<obj>) : DataColumn =
...
let private readColumn (col: DataColumn) : Type * IVector =
...
let private rowGroupsToFrame (dataFields: DataField[]) (rowGroupColumns: DataColumn[][]) : Frame<int, string> =
